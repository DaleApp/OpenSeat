...
## Context
OpenSeat is a carpooling web app for closed communities (UNC Chapel Hill). The project has the base ready (types, Firebase, auth, mock data, Tailwind config) but all UI components (~35) and pages (~15) are missing. The goal is a functional demo on a phone. All UI text is in English.
...
- `src/components/ui/BottomNav.tsx` — 4 fixed tabs: **Home, Search, Publish, Profile**, usePathname() for active state, 64px height
...
Modify:
- `src/lib/db.ts` — enhance searchRides() filters
...
- `src/app/(main)/ride/[id]/rate/page.tsx` — post-ride rating shows badge unlock animation for "Icebreaker" badge
...
- **All UI in English.** The app targets UNC Chapel Hill — all text, labels, buttons, and messages in English.
